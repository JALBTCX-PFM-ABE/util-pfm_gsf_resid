       			...
       			double lon2, double *az1, double *az2, double *s )
{
  ...
  else if(  fabs(cosphi1) < testv ) /* initial point is polar */
    {
      geo_inverse( a,rf, lat2,lon2,lat1,lon1, az1,az2,s );
      b = *az1; *az1 = *az2; *az2 = b;
      return 0;
    }
  else if( fabs(cosphi2) < testv ) /* terminal point is polar */
    {
      geo_inverse( a,rf, lat1,lon1,lat1,lon1+180.0, az1,az2,s );
      *s /= 2.0;
      *az2 = *az1 + 180.0;
      if( *az2 > 360.0 ) *az2 -= 360.0; 
      return 0;
...
/* gcc -o geod.x geod.c -DTEST_GEOD -lm */
...
int main(int argc, char **argv)
{
  ...
  for(i = 0; i < 28; i++)
    printf("<%-30.30s> %s=%2.2d %f %.9f\n",
	   DIRECT_NAMEP(i), DIRECT_ABBRVP(i), i, DIRECT_AXIS(i), DIRECT_RFLAT(i));
  ...
  printf("using:  ");
  printf("<%s> %s=%d %f %.9f\n",
	 DIRECT_NAMEP(choice), DIRECT_ABBRVP(choice), choice, DIRECT_AXIS(choice), DIRECT_RFLAT(choice));
  ...
  if (which == 0)
      ...
      while (1)
	{
	  ...
	  printf("stat = %d, ", 
		 geo_direct(DIRECT_AXIS(choice), DIRECT_RFLAT(choice), 
      ...
      while (1)
	{
	  ...
	  printf("stat = %d, ", 
		 geo_inverse(DIRECT_AXIS(choice), DIRECT_RFLAT(choice), 
